...
ui <- navbarPage(
  ...
  tabPanel("Streak Shooting by Zone: 2015-2021",
           strong("Version: 2021-07-07",style = "font-size:16px;font-family:Georgia; color:red"),
           strong("Created by Matt Bolaños | matthew.a.bolanos@gmail.com",style = "font-size:14px;font-family:Georgia; color:black"),
           br(),
           strong("2015-21 Streak Shooting by Zone",style = "font-size:30px;font-family:Georgia; color:black"),
           br(),
           strong("**CHART AND TABLE WILL TAKE A FEW MOMENTS TO LOAD**",
           ...
           sidebarLayout(sidebarPanel(pickerInput('player', 'Player', 
                                      ...
                                      pickerInput('scope', "League Average/Against Self", 
                                                  choices = c("Compared to League Average", "Compared to Player's Own Average"),
                                                  selected = "Compared to League Average", width = '100%'),
                                      withSpinner(gt_output("table1")),
                                      width = 4),
                         mainPanel(tags$style(type="text/css", "text {font-family: Consolas}"),
  ...
  tabPanel("Streak Shooting and Defender Distance: 2015-16",
           ...
           strong("Created by Matt Bolaños | matthew.a.bolanos@gmail.com",style = "font-size:14px;font-family:Georgia; color:black"),
           br(),
           strong("2015-16 Streak Shooting With Defender Distance",style = "font-size:30px;font-family:Georgia; color:black"),
           br(),
           br(),
...
server <- function(input, output) {
  ...
  #Load in player headshots
  players <- nba_players() %>% 
    filter(yearSeasonLast > 2014) %>% 
    select(urlPlayerHeadshot,namePlayer)
  
  
  #Court drawing function (h/t Owen Phillips at https://thef5.substack.com/)
  ...
  output$plot1 <- renderPlot({
    ...
    vals$scope <- as.vector(input$scope)
    
    if (vals$scope == "Compared to League Average"){
      ...
      if (nrow(player_shots) != 0){
        ...
        #Hexbin method (h/t Owen Phillips https://thef5.substack.com/)
        ...
        chart <- plot_court(court_themes$light) +
          ...
          labs(title = paste0(input$player, " After ", input$streak),
               ...
               caption = paste0("Data: nbastatR | Chart: Matt Bolaños"))+
          theme(text=element_text(size=16,  family= "Consolas"), 
                ...
                plot.subtitle = element_text(hjust = 0.5, size = 15, vjust = -1), 
                plot.caption = element_text(size = 10,hjust = .5, vjust=2),
                plot.margin = margin(0.25, -2, .35, -2, "cm"))
      ...
      input_query <- paste0("SELECT *
                FROM streak_shots_nba WHERE shooter='", player_sel, "'", "
                AND slug_season IN", year_sel, "")
      
      all_shots <- dbGetQuery(con, input_query)
      
      
      
      player_shots <- all_shots %>%
        filter(streak_col == streak_sel) %>%
        mutate(loc_x = location_x / 10,
               loc_y = location_y / 10 + hoop_center_y,
               shot_distance = distance_shot,
               shot_made_numeric = as.numeric(is_shot_made),
      ...
      if (nrow(player_shots) != 0){
        ...
        player_zone_stats <- player_averages %>%
          group_by(zone_range, name_zone) %>%
          summarize(league_pct = sum(fgm) / sum(fga))
        ...
        hex_data <- hex_data %>%
          inner_join(zone_stats, by = join_keys) %>%
          inner_join(player_zone_stats, by = join_keys)
        ...
        chart <- plot_court(court_themes$light) +
          ...
          scale_fill_distiller(direction = -1,
                               ...
                               breaks = seq(-.15, .15, .03),
                               labels = c("-15%", "-12%", "-9%", "-6%", "-3%", "0%", "+3%", "+6%", "+9%", "+12%", "+15%"),
                               paste0("FG Percentage Points vs. Player Base Zone Average")) +
          guides(fill=guide_legend(
            label.position = 'bottom',
            title.position = 'top',
            keywidth=.45,
  ...
  output$table1 <- render_gt({
      ...
      if (vals$scope == "Compared to League Average"){
        ...
        player_zone_sum <- player_zone_stats %>% 
          ungroup() %>% 
          mutate(player = zone_points_per_shot,
                 base_pct = league_pct,
                 across(base_pct, ~ round(., 2))) %>% 
          subset(select = -c(zone_points_per_shot)) %>% 
          select(zone_range,name_zone, base_pct) %>% 
          as.data.frame() 
        ...
        zone_sum <- zone_stats %>% 
          ...
          mutate(across(zone_pct, ~ round(., 2)),
                 across(zone_points_per_shot, ~ round(., 2)),
                 streak_fg_percent = zone_pct) %>% 
          select(zone_range,name_zone, streak_fg_percent, zone_points_per_shot) %>% 
          as.data.frame() %>% 
          left_join(player_zone_sum, by = c("zone_range", "name_zone")) %>% 
          mutate(dist = case_when(zone_range %like% "16-24" ~ 18,
                                  zone_range %like% "24+" ~ 24,
                                  zone_range %like% "8-16" ~ 12,
                                  zone_range %like% "Back Court" ~ 40,
                                  zone_range %like% "Less Than" ~ 4),
                 streak_diff = streak_fg_percent - base_pct) %>% 
          arrange(dist) %>% 
          subset(select=-c(dist)) %>% 
          select(zone_range, name_zone, streak_fg_percent, zone_points_per_shot, streak_diff) %>% 
          as.data.frame() 
        ...
        zone_sum %>% 
          ...
          cols_label(zone_range = "Distance",
                     name_zone = "Zone",
                     streak_fg_percent = "Streak FG%",
                     zone_points_per_shot = "Streak PPS",
                     streak_diff = "Base FG% Diff") %>% 
          data_color(
            columns = vars(zone_points_per_shot),
            colors = col_numeric(
              palette =paletteer_d(
          ...
          data_color(
            columns = vars(streak_fg_percent),
            colors = col_numeric(
              palette =paletteer_d(
                palette = "RColorBrewer::YlOrRd",
                direction  = 1
          ...
          tab_options(
            ...
            table.border.top.color = "transparent",
            data_row.padding = px(2)
          ) %>% cols_width(vars(streak_fg_percent, zone_points_per_shot, streak_diff) ~ px(60),
                           vars(zone_range) ~ px(100),
                           vars(name_zone) ~ px(75)) %>%
          opt_align_table_header(align = "left")  %>% 
          tab_header(
            title = paste0("  FG% and Points Per Shot by Zone After ", vals$streak),
            subtitle = paste0("  Base FG% Diff = Streak Zone FG% - Base Zone FG%"))
